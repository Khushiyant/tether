...
def linear_lif_fwd_kernel(
    X_ptr,
    W_ptr,  # Inputs: X (T, B, In), W (Out, In)
    ...
    if V_init_ptr is not None:
        ...
        v = tl.load(
            V_init_ptr + offs_v, mask=(mask_b[:, None] & mask_o[None, :]), other=0.0
        )
    ...
    for t in range(T):
        ...
        for k in range(0, In, BLOCK_I):
            ...
            x_ptrs = (
                ...
                + offs_i[None, :] * stride_xi
            )
            x_tile = tl.load(
                x_ptrs, mask=(mask_b[:, None] & mask_i[None, :]), other=0.0
            )
            ...
            w_tile = tl.load(
                w_ptrs, mask=(mask_o[:, None] & mask_i[None, :]), other=0.0
            )
        ...
        s_ptrs = (
            S_out_ptr
            + t * stride_st
            + offs_b[:, None] * stride_sb
            + offs_o[None, :] * stride_so
        )
