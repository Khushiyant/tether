...
def get_mnist_loaders(batch_size=64, n_steps=10, framework="tether"):
    ...
    if framework == "tether":
        ...
        train_dataset = SpikingDatasetWrapper(
            train_dataset, encode_fn=lambda x: rate_encoding(x, n_steps=n_steps)
        )
        test_dataset = SpikingDatasetWrapper(
            test_dataset, encode_fn=lambda x: rate_encoding(x, n_steps=n_steps)
        )
...
def compare_results(results: List[BenchmarkResult]) -> Dict:
    ...
    for task, task_results in by_task.items():
        ...
        task_comparison = {"baseline": baseline.framework, "frameworks": {}}
...
def main():
    ...
    parser.add_argument("--n-steps", type=int, default=10, help="Number of time steps")
    parser.add_argument(
    ...
    if device == "cpu":
        print(
            "WARNING: CUDA not available. Benchmarks will be slow and less meaningful."
        )
    ...
    frameworks_to_test = [
        fw for fw in args.frameworks if FRAMEWORKS_AVAILABLE.get(fw, False)
    ]
    ...
    for task, comp in comparison.items():
        ...
        for fw, stats in comp["frameworks"].items():
            print(
                f"  {fw:15s}: {stats['speedup']:>6s} | Acc: {stats['accuracy']:>6.2f}% | Time: {stats['time']:>6.1f}s"
            )
